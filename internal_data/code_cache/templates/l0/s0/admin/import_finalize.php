...
return array('macros' => array('notes' => function($__templater, array $__arguments, array $__vars)
{
	...
	if ($__vars['notes']['runTime'] > 3600) {
		$__compilerTemp1 .= '
			' . '' . $__templater->fn('number', array($__vars['notes']['runTime'] / 3600, 2, ), true) . ' hours' . '
		';
	} else if ($__vars['notes']['runTime'] >= 120) {
		$__compilerTemp1 .= '
			' . '' . $__templater->fn('number', array($__vars['notes']['runTime'] / 60, 2, ), true) . ' minutes' . '
		';
	} else {
		$__compilerTemp1 .= '
		...
		foreach ($__vars['notes']['totals'] AS $__vars['total']) {
			...
			if ($__vars['total']['total']) {
				...
				if ($__vars['total']['time'] > 3600) {
					...
					$__compilerTemp2 .= '
							' . $__templater->fn('parens', array('' . $__templater->fn('number', array($__vars['total']['time'] / 60, 2, ), false) . ' minutes', ), true) . '
						';
				} else {
					$__compilerTemp2 .= '
