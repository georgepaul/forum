...
b.proxy(this,"toggleActive"));this.$title=XF.findRelativeIf(this.options.title,this.$target);this.$active=this.$target.find(this.options.active);this.$menu=XF.findRelativeIf(this.options.menu,this.$target);this.$menuContent=this.$menu.find(this.options.menuContent);this.$menuContent.on("click","[data-prefix-id]",b.proxy(this,"prefixClick"));this.template=this.$menuContent.find('script[type="text/template"]').html();this.template||(console.error("No template could be found"),this.template="");if(this.options.href){var a=
this.options.listenTo?XF.findRelativeIf(this.options.listenTo,this.$target):b([]);if(a.length)a.on("change",b.proxy(this,"loadPrefixes"));else console.error("Cannot load prefixes dynamically as no element set to listen to for changes")}this.initMenu();if(a=parseInt(this.$select.val(),10))this.initialPrefixId=a,this.selectPrefix(a);this.$target.closest("form").on("reset",b.proxy(function(){this.reset()},this))}else console.error("No select matching %s",this.options.select)},initMenu:function(){var a=
