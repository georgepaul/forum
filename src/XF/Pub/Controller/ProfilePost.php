...
class ProfilePost extends AbstractController
{
	...
	public function actionLike(ParameterBag $params)
	{
		...
		return $likePlugin->actionToggleLike(
			$profilePost,
			$this->buildLink('profile-posts/like', $profilePost),
			$this->buildLink('profile-posts', $profilePost),
			$this->buildLink('profile-posts/likes', $profilePost)
		);
	}
	...
	public function actionLikes(ParameterBag $params)
	{
		...
		return $likePlugin->actionLikes(
			$profilePost,
			['profile-posts/likes', $profilePost],
			null,
			$breadcrumbs
		);
	}
	...
	public function actionCommentsLike(ParameterBag $params)
	{
		$comment = $this->assertViewableComment($params->profile_post_comment_id);
		...
		return $likePlugin->actionToggleLike(
			$comment,
			$this->buildLink('profile-posts/comments/like', $comment),
			$this->buildLink('profile-posts/comments', $comment),
			$this->buildLink('profile-posts/comments/likes', $comment)
		);
	}

	public function actionCommentsLikes(ParameterBag $params)
	{
		$comment = $this->assertViewableComment($params->profile_post_comment_id);
