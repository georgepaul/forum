...
trait vBulletinBlogTrait
{
	...
	protected function getStepConfigDefault()
	{
		return parent::getStepConfigDefault() + [
				'blogUsers' => [
					'parent_node_id' => 0,
					'parent_node_title' => \XF::phrase('import_blogs')
				],
				'blogAttachments' => [
	...
	//	die(PHP_EOL . 'Blog users...' . PHP_EOL);
		return $this->sourceDb->fetchOne($this->prepareImportSql($this->prefix, "SELECT MAX(bloguserid) FROM blog_user")) ?: 0;
	}
	...
	public function stepBlogUsers(StepState $state, array $stepConfig, $maxTime, $limit = 100)
	{
		...
		$blogUsers = $this->sourceDb->fetchAllKeyed($this->prepareImportSql($this->prefix, "
			SELECT
				bu.bloguserid,
	...
	/**
	 * When $retainIds is set, creates a category node in order to store all imported
	 * blogs, which would otherwise be scattered all over the node tree.
	 *
	...
	protected function getBlogsParentNodeId(array $stepConfigBlogUsers)
	{
		...
		{
			// When retaining IDs, we don't have a node to import into, so make one and remember if
			if (empty($this->session->extra['blogUsersParentNodeId']))
			{
				...
				$this->session->extra['blogUsersParentNodeId'] = $parentNode->node_id;
			}
	...
	{
		return $this->sourceDb->fetchOne($this->prepareImportSql($this->prefix, "SELECT MAX(blogid) FROM blog")) ?: 0;
	}
	...
	public function stepBlogEntries(StepState $state, array $stepConfig, $maxTime, $limit = 1000)
	{
		...
		$blogEntries = $this->sourceDb->fetchAllKeyed($this->prepareImportSql($this->prefix, "
			SELECT
				blog.*, blog_text.*,
				user.username
			FROM blog AS
	...
	{
		return $this->sourceDb->fetchOne($this->prepareImportSql($this->prefix, "SELECT MAX(attachmentid) FROM blog_attachment")) ?: 0;
	}
	...
	public function stepBlogAttachments(StepState $state, array $stepConfig, $maxTime, $limit = 1000)
	{
		...
		$attachments = $this->sourceDb->fetchAllKeyed($this->prepareImportSql($this->prefix, "
			SELECT
		...
		", $limit), 'attachmentid', [$state->startAfter, $state->end]);
	...
	public function stepBlogComments(StepState $state, array $stepConfig, $maxTime, $limit = 500)
	{
		...
		$blogs = $this->sourceDb->fetchPairs($this->prepareImportSql($this->prefix, "
		...
		foreach ($blogs AS $blogId => $firstBlogTextId)
		{
			 ...
			 * in most cases...
			 */
			$blogComments = $this->sourceDb->fetchAll($this->prepareImportSql($this->prefix, "
				SELECT
					blog_text.*,
					user.username,
	...
	{
		$moderators = $this->sourceDb->fetchAll($this->prepareImportSql($this->prefix, "
